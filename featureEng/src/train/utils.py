...
def _build_datasets(params):
    ...
    LOGGER.info("%d datapoints in datasets '%s'", len(train_x), params['dataset_file'])
    LOGGER.info("%d datapoints in test datasets '%s'", len(test_x), params['dataset_file'])
...
def save_results_cc_val(output_path, params, f1_micro, f1_macro, f1_weight, acc, precision, recall):
    f = open(output_path + "/results" + '_micro_val' + ".txt", "a")
    f.write(str(f1_micro) + '\n')
    f.close()

    f = open(output_path + "/results" + '_macro_val' + ".txt", "a")
    f.write(str(f1_macro) + '\n')
    f.close()

    f = open(output_path + "/results" + '_f1weight_val' + ".txt", "a")
    f.write(str(f1_weight) + '\n')
    f.close()

    f = open(output_path + "/results" + '_acc_val' + ".txt", "a")
    f.write(str(acc) + '\n')
    f.close()

    f = open(output_path + "/results" + '_precision_val' + ".txt", "a")
    f.write(str(precision) + '\n')
    f.close()

    f = open(output_path + "/results" + '_recall_val' + ".txt", "a")
    f.write(str(recall) + '\n')
    f.close()
