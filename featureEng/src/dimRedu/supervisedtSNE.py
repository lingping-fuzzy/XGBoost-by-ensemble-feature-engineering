...
class SupervisedTSNE(ClassNamePrefixFeaturesOutMixin, TransformerMixin, BaseEstimator):
    ...
    def _fit(self, X, y=None, skip_num_points=0):
        ...
        # added by ---kong -here change the P based on the label y
        # --------------------------------------------------------------------
        if y is not None:
            P = self._supervised_joint_probabilities(P, y)
    ...
    def _supervised_joint_probabilities(self, P, y):
        # Ensure P is in the correct sparse format
        P = csr_matrix(P)

        # Convert to LIL format for efficient modification
        label_similarity = P.tolil()

        # Get the non-zero indices
        P_coo = P.tocoo()

        # Iterate over non-zero elements
        for i, j, value in zip(P_coo.row, P_coo.col, P_coo.data):
            if y[i] == y[j]:
                # Increase probability for same class
                label_similarity[i, j] *= 1.2  # or another factor
            else:
                # Decrease probability for different class
                label_similarity[i, j] *= 0.8  # or another factor

        # Convert back to CSR format for efficient arithmetic operations
        label_similarity = label_similarity.tocsr()

        # Normalize label_similarity to ensure it's a valid probability distribution
        label_similarity.data /= np.sum(label_similarity.data)
        label_similarity = csr_matrix(label_similarity)
    ...
    def transform(self, X):
        ...
        #     raise RuntimeError("You must fit the model before transforming new data.")
        ...
        if self.method != "exact":
            ...
            for i in range(X.shape[0]):
                X_new[i] = np.dot(self.embedding_[indices[i]].T, weights[i])
        else:
            print('results are inaccurate, if you use exact method for transform')
            raise -1
